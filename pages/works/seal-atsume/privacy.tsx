...
export default function Page() {
  return (
    ...
    >
      ...
      <h2>保存と削除</h2>
      <p>
        ステッカー画像、ボードの配置、表示テーマをアプリ内に保存します。作品はアプリ内で削除できます。ボードで使用中の画像は、そのボードに必要な間は残ります。写真アプリの元画像や書き出した画像は変更・削除しません。端末の設定によりアプリのデータがAppleのバックアップに含まれる場合があります。
      </p>
      <h2>iCloudによる同期</h2>
      <p>
        端末でiCloudにサインインしている場合、ステッカー画像とボードの内容は、AppleのiCloud（CloudKit）にある利用者自身のプライベート領域にも保存され、同じApple IDでサインインしているiPhone・iPadの間で同期されます。このデータは利用者のiCloudストレージを使用し、提供者が閲覧したり、提供者のサーバーへ送信したりすることはありません。アプリ内で作品を削除すると、iCloudと他の端末からも削除されます。
      </p>
      <p>
        アプリを削除しても、iCloudに同期済みの作品は残り、再インストールすると戻ります。iCloudを使っていない場合は、アプリを削除するとアプリ内の作品も削除されます。iCloudに保存されたデータは、iPhoneの「設定」＞ユーザー名＞「iCloud」のストレージ管理から削除できます。iCloudにはAppleのプライバシーポリシーが適用されます。
      </p>
      <h2>写真・カメラへのアクセス</h2>
      <p>
